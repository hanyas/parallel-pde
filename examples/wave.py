...
def get_transition_model(
    pde: PDE, x_grid: jax.Array, q: float = 10.0,
    sigma: float = 1000.0, nell: float = 1.0
):
    dt = pde.dt
    dx = pde.dx
...
def wave_equation_fn(pde: PDE, s: jax.Array):
    J = int(len(s) / 3) + 1
    u = s[:J - 1]
...
if __name__ == "__main__":
    ...
    pde = PDE(min_x, max_x, 0.0, 0.0, u_0, max_t, dx, dt)

    x_grid_exact = jnp.linspace(min_x, max_x, int(max_t / dt))
    ...
    observation_model = FunctionalModel(
        lambda s: wave_equation_fn(pde, s),
    ...
    mask = jnp.kron(jnp.array([1.0, 0.0, 0.0]), jnp.eye(x_size - 1))
    ...
    ieks_frames = [0, int(0.15 * max_t / dt), int(0.5 * max_t / dt), int(0.85 * max_t / dt)]
    ...
    for idx, frame_idx in enumerate(ieks_frames):
        ...
        ax.plot(x_grid_exact, u_exact(x_grid_exact, exact_frames[idx]), 'k--', linewidth=2.5, label='Exact')
        if idx == 0:
